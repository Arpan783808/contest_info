...
export async function fetchLcData(username) {
    try {
        ...
        var totalrating = data.matchedUser.submitStats.acSubmissionNum[1].count * 1000 + data.matchedUser.submitStats.acSubmissionNum[2].count * 1300 + data.matchedUser.submitStats.acSubmissionNum[3].count * 1600;
...
export async function fetchLc(username,prev_data) {
    try {
        ...
        var score = score = Math.ceil(problems + avgrating / 100 + ratingchange / 10 + contest * 10);
